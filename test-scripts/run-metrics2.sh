...
echo 'POD,CPU(Mi),MEMORY(Mi),JVM(MI),TIMESTAMP' > ./$TEST_ID/kafka-3-output.csv
echo 'POD,CPU(Mi),MEMORY(Mi),JVM(MI),TIMESTAMP' > ./$TEST_ID/kafka-4-output.csv
linecpu=0
totalcpu=0
linemem=0
totalmem=0
...
do
  ...
  JVM=$(curl -s  $PROM_URL?query="sum(jvm_memory_bytes_used\{namespace='$NAMESPACE',kubernetes_pod_name=~'my-cluster-kafka-3',strimzi_io_name='my-cluster-kafka'\})" | jq -r ".data.result"[].value |  jq -r '.[]' | sed 1d)  
  oc adm top pods -n $NAMESPACE -l 'statefulset.kubernetes.io/pod-name in (my-cluster-kafka-3)' | sed 1d |awk -F' '  -v jvm=$JVM '{print $0, jvm"\t" }' | awk -F ' ' -v timestamp=$(date +%F:%T)  '{print $0 timestamp}' | awk -F' ' 'BEGIN{OFS=",";} {gsub(/[Mi]/,""); print $1,$2,$3,$4,$5;}' | awk '{gsub(/m,/,","); print }' >> ./$TEST_ID/output.csv 
  JVM=$(curl -s  $PROM_URL?query="sum(jvm_memory_bytes_used\{namespace='$NAMESPACE',kubernetes_pod_name=~'my-cluster-kafka-4',strimzi_io_name='my-cluster-kafka'\})" | jq -r ".data.result"[].value |  jq -r '.[]' | sed 1d)  
  oc adm top pods -n $NAMESPACE -l 'statefulset.kubernetes.io/pod-name in (my-cluster-kafka-4)' | sed 1d |awk -F' '  -v jvm=$JVM '{print $0, jvm"\t" }' | awk -F ' ' -v timestamp=$(date +%F:%T)  '{print $0 timestamp}' | awk -F' ' 'BEGIN{OFS=",";} {gsub(/[Mi]/,""); print $1,$2,$3,$4,$5;}' | awk '{gsub(/m,/,","); print }' >> ./$TEST_ID/output.csv 
  if 
    [ $counter -gt $startAvg ]
  then 
...
sed -n '/kafka-3/p' ./$TEST_ID/output.csv >> ./$TEST_ID/kafka-3-output.csv
sed -n '/kafka-4/p' ./$TEST_ID/output.csv >> ./$TEST_ID/kafka-4-output.csv
